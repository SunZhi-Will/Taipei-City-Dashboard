	...
	return {
		...
		presentation: {
			...
			slides: toArray(scene?.presentation?.slides).map((slide, index) => ({
				...
				chartType: slide?.chartType || undefined,
				mapQuery: typeof slide?.mapQuery === 'string' ? slide.mapQuery : '',
				mapLng: Number.isFinite(Number(slide?.mapLng)) ? Number(slide.mapLng) : undefined,
				mapLat: Number.isFinite(Number(slide?.mapLat)) ? Number(slide.mapLat) : undefined,
